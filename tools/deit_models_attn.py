...
class MyVisionTransformer(VisionTransformer):
    ...
    def forward_feature_patch_embed_all_one(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
    ...
    def forward_features_all(self, x: torch.Tensor) -> torch.Tensor:
        x = self.forward_feature_patch_embed_all_one(x)
        for blk in self.blocks:
            x, _ = blk(x, policy=None)
        x = self.norm(x)
        return x
